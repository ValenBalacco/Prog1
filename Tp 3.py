...
for letter in word:
    if letter in vocal:
        if letter not in vocal_in_word:
...
objetive = float(input("¿Cuánto dinero desea ahorrar? "))
total_saving = 0

while total_saving  < objetive:
    cant = float(input("Ingrese la cantidad que desea ahorrar: "))
    if cant < 0:
        print("La cantidad ingresada debe ser positiva.")
        continue
    total_saving += cant

print(f"¡Felicidades! Ha alcanzado su objetivo de ahorro de ${objetive}.")
